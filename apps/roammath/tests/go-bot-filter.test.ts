...
// Ported from ParkMath's tests/go-bot-filter.test.ts — the underlying module is generic (no
// airport/country-specific logic), so the same fixtures and assertions apply verbatim.

// A real Chrome-on-macOS click on a `target="_blank" rel="noopener noreferrer"` CTA: full UA + the
// Fetch Metadata / Accept-Language headers every modern browser attaches to a top-level navigation, no
...
describe("isLikelyBot — self-identifying bots (Googlebot-class)", () => {
  ...
  it("flags AhrefsBot", () => {
    expect(
      isLikelyBot({ ...REAL_CHROME_CLICK, userAgent: "Mozilla/5.0 (compatible; AhrefsBot/7.0; +http://ahrefs.com/robot/)" }),
    ).toBe(true);
  });
...
describe("isLikelyBot — generic scrapers spoofing a full browser UA", () => {
...
describe("isNearDuplicateClick — same-key repeats within the window are suppressed", () => {
  ...
  it("lets the first fire through, suppresses an immediate repeat", () => {
    const key = "81.174.10.20:spain:esim:airalo";
  ...
  it("treats different country/target/ip combinations as independent", () => {
    expect(isNearDuplicateClick("81.174.10.20:spain:esim:airalo")).toBe(false);
    expect(isNearDuplicateClick("81.174.10.20:france:esim:airalo")).toBe(false);
    expect(isNearDuplicateClick("9.9.9.9:spain:esim:airalo")).toBe(false);
  });
});
