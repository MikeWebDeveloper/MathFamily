...
// Ported from ParkMath's lib/go-bot-filter.ts (same battle-tested pattern, generalized — nothing
// airport-specific in the logic itself). Forwarding the visitor's real User-Agent to Umami (done in
// go-analytics.ts) lets Umami's own heuristic drop self-identifying bots (Googlebot, AhrefsBot, ...)
// fine — but generic scrapers/scanners that SPOOF a full desktop-browser UA string (no "bot"/"crawler"
// substring to match) sail straight through that heuristic. The two checks below close that gap. Kept
// in its own module (not inline in the route) so it's unit-testable without going through Next's
// route-handler/`after()` request scope.
//
 ...
 *   1. The UA self-identifies (or fingerprints) as a bot/scraper/HTTP-library.
 ...
 *      scrapers that spoof a generic Chrome/Safari UA string routinely forget to fake them.
 *  Deliberately does NOT check Referer: RoamMath's affiliate CTAs use `rel="noopener noreferrer"` (so
 *  we don't leak the affiliate path to the partner/analytics on the outbound hop) — genuine human
 *  clicks arrive with no Referer too, so using its absence as a bot signal would zero out real
 *  conversions, not just bots. */
...
// Best-effort, in-process de-dupe for near-duplicate fires (double-taps / retries landing on the same
// warm serverless instance within a few seconds). Not a distributed rate-limiter — it only covers
// repeats on one warm lambda.
