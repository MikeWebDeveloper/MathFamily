...
test("home parking search turns into a tracked affiliate link once an airport is chosen", async ({ page }) => {
  ...
  await search.getByLabel("Drop-off date").fill("2026-12-07");
  await search.getByLabel("Return date").fill("2026-12-12");
  await expect(cta).toHaveAttribute("href", /depart%3DLGW/);
  await expect(cta).toHaveAttribute("href", /out%3D2026-12-07/);
  await expect(cta).toHaveAttribute("href", /in%3D2026-12-12/);
});
